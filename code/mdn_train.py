...
def train(data_loader, model, optimizer, epoch, bm_mask):
    model.train()
    total_am, subgraph_am, node_am, esl_am = AverageMeter(), AverageMeter(), AverageMeter(), AverageMeter()
    for n_iter, (input_data, label_confidence, label_start, label_end) in enumerate(data_loader):
        ...
        edge_loss = edge_loss_func(confidence_map[:,0], gt_iou_map, bm_mask)
        loss = subgraph_loss + node_loss + edge_loss
        ...
        edge_am.update(edge_loss.detach())

    print("[Epoch {0:03d}]\tLoss {1:.2f} = {2:.2f} + {3:.2f} + {4:.2f} (train)".format(
        epoch, total_am.avg(), subgraph_am.avg(), node_am.avg(), edge_am.avg()))
...
def test(data_loader, model, epoch, bm_mask):
    ...
    print("[Epoch {0:03d}]\tLoss {1:.2f} = {2:.2f} + {3:.2f} + {4:.2f} (Val)".format(
        epoch, total_am.avg(), subgraph_am.avg(), node_am.avg(), edge_am.avg()))
