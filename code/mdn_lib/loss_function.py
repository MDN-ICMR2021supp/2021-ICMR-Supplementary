...
def edge_loss_func(pred_score, gt_iou_map, mask, lambda1 = 500000):

    emask = ((gt_iou_map < 0.9) & (gt_iou_map > 0)).float()
    emask = emask * mask

    
    loss = F.mse_loss(pred_score * emask, gt_iou_map * emask)

    loss = lambda1 * torch.sum(loss) / torch.sum(emask)
